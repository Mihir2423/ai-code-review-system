...
async function upsertRepositories(repos, installationRecordId, userId) {
    const results = await Promise.all(repos.map((repo) => prisma.repository.upsert({
    ...
    if (installationRecord) {
        await Promise.all(results.map((repository) => addJob(repoIndexQueue, 'repo-index', {
        ...
        if (action === 'created') {
            ...
            if (account) {
                ...
                if (repositories.length) {
                    ...
                    logger.info({ count: repositories.length, installationId: installation.id }, 'Initial repositories synced');
                }
            }
            else {
    ...
    if (event === 'installation_repositories') {
        ...
        if (repositories_removed?.length) {
            await prisma.repository.deleteMany({
                where: {
                    githubId: { in: repositories_removed.map((r) => String(r.id)) },
                    installationId: installationRecord.id,
                },
            });
            logger.info({ count: repositories_removed.length, installationId: installation.id }, 'Repositories removed');
        }
    }
