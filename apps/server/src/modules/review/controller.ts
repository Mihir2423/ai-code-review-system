...
import { getReviewById, getReviewEvents, getUserReviewHistory } from './service.js';
    ...
    try {
        ...
        if (!review) {
            res.status(StatusCodes.NOT_FOUND).json(
                new FailResponse.Builder().withMessage('Review not found').withContent({ error: 'Review not found' }).build(),
            );
            return;
        }
